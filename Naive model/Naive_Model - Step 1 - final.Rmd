...
a_dp_grp1_wd <- h1_train_1 %>%
      select(days_prior, prod_group_perc, day_of_week, OTB_cxl_rate) %>%
      group_by(days_prior, prod_group_perc, day_of_week) %>%
      summarise(predict_cxl_rate = mean(OTB_cxl_rate))
a_dp_grp1_wd_test <- hot1_test_1 %>%
      select(days_prior, prod_group_perc, day_of_week, OTB, OTB_to_be_cxl, OTB_cxl_rate) %>%
      group_by(days_prior, prod_group_perc, day_of_week) %>%
      summarise(true_cxl_rate = mean(OTB_cxl_rate))
...
a_dp_grp2_wd <- h1_train_2 %>%
      select(days_prior, prod_group_behav, day_of_week, OTB, OTB_to_be_cxl, OTB_cxl_rate) %>%
...
a_dp_grp3_wd <- h1_train_3 %>%
      select(days_prior, prod_group_cxl_rate, day_of_week, OTB, OTB_to_be_cxl, OTB_cxl_rate) %>%
...
a_dp_grp2_dtp <- h1_train_2 %>%
      select(days_prior, prod_group_behav, day_type, OTB, OTB_to_be_cxl, OTB_cxl_rate) %>%
...
a_dp_grp3_dtp <- h1_train_3 %>%
      select(days_prior, prod_group_cxl_rate, day_type, OTB, OTB_to_be_cxl, OTB_cxl_rate) %>%
...
n_dp_grp1_dtp <- h2_train_1 %>%
      select(days_prior, prod_group_perc, day_type, OTB, OTB_to_be_cxl, OTB_cxl_rate) %>%
      group_by(days_prior, prod_group_perc, day_type) %>%
      summarise(predict_cxl_rate = mean(OTB_cxl_rate))
...
```{r}
# MAE, MAPE, MASE
h1_dp <- left_join(left_join(a_dp_mae,a_dp_mape, by="dp_range"), a_dp_mase, by="dp_range")
```
...
```{r}
# MAE, MAPE, MASE
h1_dp_g1_wd <- left_join(left_join(a_dp_grp1_wd_mae,a_dp_grp1_wd_mape, by=c("dp_range")), a_dp_grp1_wd_mase, by="dp_range") %>% 
...
```{r}
# MAE, MAPE, MASE
h1_dp_g2_wd <- left_join(left_join(a_dp_grp2_wd_mae,a_dp_grp2_wd_mape, by=c("dp_range")), a_dp_grp2_wd_mase, by="dp_range")
```
#### Grouping method 3
```{r}
# forecast otb to survive
...
```{r}
# MAE, MAPE, MASE
h1_dp_g3_wd <-left_join(left_join(a_dp_grp3_wd_mae,a_dp_grp3_wd_mape, by=c("dp_range")), a_dp_grp3_wd_mase, by="dp_range") %>% 
  select(dp_range, meanAE, meanAPE, meanASE)
```
...
```{r}
# MAE, MAPE, MASE
h1_dp_g1_dt <-left_join(left_join(a_dp_grp1_dtp_mae,a_dp_grp1_dtp_mape, by=c("dp_range")), a_dp_grp1_dtp_mase, by="dp_range") %>% 
  select(dp_range,  meanAE, meanAPE, meanASE)
```
...
```{r}
# MAE, MAPE, MASE
h1_dp_g2_dt <-left_join(left_join(a_dp_grp2_dtp_mae,a_dp_grp2_dtp_mape, by=c("dp_range")), a_dp_grp2_dtp_mase, by="dp_range") %>% 
  select(dp_range,  meanAE, meanAPE, meanASE)
```
#### Grouping method 3
```{r}
...
```{r}
# MAE, MAPE, MASE
h1_dp_g3_dt <-left_join(left_join(a_dp_grp3_dtp_mae,a_dp_grp3_dtp_mape, by=c("dp_range")), a_dp_grp3_dtp_mase, by="dp_range") %>% 
  select(dp_range, meanAE, meanAPE, meanASE)
```

```{r}
join_all <- h1_dp %>%
  left_join(h1_dp_g1_wd, by = "dp_range") %>%
  left_join(h1_dp_g3_wd, by = "dp_range") %>%
  left_join(h1_dp_g1_dt, by = "dp_range") %>%
  left_join(h1_dp_g3_dt, by = "dp_range")



join_all <- join_all %>% mutate(dp_range = case_when(dp_range == '1_7' ~ '1-7',
                                          dp_range == '8_14' ~ '8-14',
                                          dp_range == '15_21' ~ '15-21',
                                          dp_range == '22_28' ~ '22-28',
                                          dp_range == '29_32' ~ '29-32'))
  
  
hotel1 <- join_all %>%
  arrange(meanASE.x.x) %>%
  kable(col.names = c("Days Prior", "MAE", "MAPE", "MASE", "MAE", "MAPE", "MASE", "MAE", "MAPE", "MASE", "MAE", "MAPE", "MASE", "MAE", "MAPE", "MASE")) %>%
  kable_styling(bootstrap_options = c("condensed", "striped", "bordered")) %>%
  column_spec(1:15, color = "black") %>%
  add_header_above(c(" " = 1, "Days Prior" = 3, "Group 1 and DOW" = 3, "Group 3 and DOW" = 3, "Group 1 and Day Type" = 3,  "Group 3 and Day Type" = 3)) %>%
  add_header_above(c("Atlanta Hotel" = 16))
```

###__In Sample Error Rates__

```{r}
# forecast otb to survive
a_dp_fcs <- left_join(h1_train, a_dp_mod, by="days_prior") %>% 
  mutate(fcst_svv = OTB-OTB*predict_cxl_rate)
```
...
```{r}
# forecast otb to survive
a_dp_grp1_wd_fcs <- left_join(h1_train_1, a_dp_grp1_wd_mod, by=c("days_prior", "day_of_week", "prod_group_perc")) %>% 
  mutate(fcst_svv = OTB-OTB*predict_cxl_rate)
a_dp_grp1_wd_fcs
```
...
```{r}
# forecast otb to survive
a_dp_grp2_wd_fcs <- left_join(h1_train_2, a_dp_grp2_wd_mod, by=c("days_prior", "day_of_week", "prod_group_behav")) %>% 
  mutate(fcst_svv = OTB-OTB*predict_cxl_rate)
a_dp_grp2_wd_fcs
```
...
```{r}
# forecast otb to survive
a_dp_grp3_wd_fcs <- left_join(h1_train_3, a_dp_grp3_wd_mod, by=c("days_prior", "day_of_week", "prod_group_cxl_rate")) %>% 
  mutate(fcst_svv = OTB-OTB*predict_cxl_rate)
a_dp_grp3_wd_fcs
```
...
```{r}
# forecast otb to survive
a_dp_grp1_dtp_fcs <- left_join(h1_train_1, a_dp_grp1_dtp_mod, by=c("days_prior", "day_type", "prod_group_perc")) %>% 
  mutate(fcst_svv = OTB-OTB*predict_cxl_rate)
a_dp_grp1_dtp_fcs
```
...
```{r}
# forecast otb to survive
a_dp_grp2_dtp_fcs <- left_join(h1_train_2, a_dp_grp2_dtp_mod, by=c("days_prior", "day_type", "prod_group_behav")) %>% 
  mutate(fcst_svv = OTB-OTB*predict_cxl_rate)
a_dp_grp2_dtp_fcs
```
...
```{r}
# forecast otb to survive
a_dp_grp3_dtp_fcs <- left_join(h1_train_3, a_dp_grp3_dtp_mod, by=c("days_prior", "day_type", "prod_group_cxl_rate")) %>% 
  mutate(fcst_svv = OTB-OTB*predict_cxl_rate)
a_dp_grp3_dtp_fcs
```
...
hotel1_in_sample <- join_all %>%
  arrange(meanASE.x.x) %>%
  kable(col.names = c("Days Prior", "MAE", "MAPE", "MASE", "MAE", "MAPE", "MASE", "MAE", "MAPE", "MASE", "MAE", "MAPE", "MASE", "MAE", "MAPE", "MASE")) %>%
  kable_styling(bootstrap_options = c("condensed", "striped", "bordered")) %>%
  column_spec(1:15, color = "black") %>%
  add_header_above(c(" " = 1, "Days Prior" = 3, "Group 1 and DOW" = 3, "Group 3 and DOW" = 3, "Group 1 and Day Type" = 3,  "Group 3 and Day Type" = 3)) %>%
  add_header_above(c("Atlanta Hotel - In Sample" = 16)) 
```
...
```{r}
# MAE, MAPE, MASE
h2_dp <- left_join(left_join(n_dp_mae,n_dp_mape, by="dp_range"), n_dp_mase, by="dp_range")
```
...
```{r}
# forecast otb to survive
n_dp_grp1_wd_fcs <- left_join(hot2_test_1, n_dp_grp1_wd_mod, by=c("days_prior")) %>% 
  mutate(fcst_svv = OTB-OTB*predict_cxl_rate)
n_dp_grp1_wd_fcs
```
...
```{r}
# MAE, MAPE, MASE
h2_dp_g1_wd <-left_join(left_join(n_dp_grp1_wd_mae,n_dp_grp1_wd_mape, by=c("dp_range")), n_dp_grp1_wd_mase, by="dp_range") %>% 
  select(dp_range, meanAE, meanAPE, meanASE)
```
...
```{r}
# MAE, MAPE, MASE
h2_dp_g2_wd <-left_join(left_join(n_dp_grp2_wd_mae,n_dp_grp2_wd_mape, by=c("dp_range")), n_dp_grp2_wd_mase, by="dp_range") %>% 
  select(dp_range, meanAE, meanAPE, meanASE)
```
...
```{r}
# MAE, MAPE, MASE
h2_dp_g1_dt <-left_join(left_join(n_dp_grp1_dtp_mae,n_dp_grp1_dtp_mape, by=c("dp_range")), n_dp_grp1_dtp_mase, by="dp_range") %>% 
  select(dp_range, meanAE, meanAPE, meanASE)
```
#### Grouping method 2
```{r}
...
```{r}
# MAE, MAPE, MASE
h2_dp_g2_dt <-left_join(left_join(n_dp_grp2_dtp_mae,n_dp_grp2_dtp_mape, by=c("dp_range")), n_dp_grp2_dtp_mase, by="dp_range") %>% 
  select(dp_range,  meanAE, meanAPE, meanASE)
```
...
```{r}
hotel2 <- h2_dp %>%
  left_join(h2_dp_g1_wd, by = "dp_range") %>%
  left_join(h2_dp_g2_wd, by = "dp_range") %>%
  left_join(h2_dp_g1_dt, by = "dp_range") %>%
  left_join(h2_dp_g2_dt, by = "dp_range") %>%
  mutate(dp_range = case_when(dp_range == '1_7' ~ '1-7',
                                          dp_range == '8_14' ~ '8-14',
                                          dp_range == '15_21' ~ '15-21',
                                          dp_range == '22_28' ~ '22-28',
                                          dp_range == '29_32' ~ '29-32')) %>%
  ...
  kable_styling(bootstrap_options = c("condensed", "striped", "bordered")) %>%
  column_spec(1:15, color = "black") %>%
  add_header_above(c(" " = 1, "Days Prior" = 3, "Group 1 and DOW" = 3, "Group 2 and DOW" = 3, "Group 1 and Day Type" = 3, "Group 2 and Day Type" = 3)) %>%
  add_header_above(c(" " = 1, "New York Hotel" = 15))
```
...
```{r}
# forecast otb to survive
n_dp_fcs <- left_join(h2_train, n_dp_mod, by="days_prior") %>% 
  mutate(fcst_svv = OTB-OTB*predict_cxl_rate)
```
...
```{r}
# forecast otb to survive
n_dp_grp1_wd_fcs <- left_join(h2_train_1, n_dp_grp1_wd_mod, by=c("days_prior")) %>% 
  mutate(fcst_svv = OTB-OTB*predict_cxl_rate)
n_dp_grp1_wd_fcs
```
...
```{r}
# forecast otb to survive
n_dp_grp2_wd_fcs <- left_join(h2_train_2, n_dp_grp2_wd_mod, by=c("days_prior", "day_of_week", "prod_group_behav")) %>% 
  mutate(fcst_svv = OTB-OTB*predict_cxl_rate)
n_dp_grp2_wd_fcs
```
...
```{r}
# forecast otb to survive
n_dp_grp1_dtp_fcs <- left_join(h2_train_1, n_dp_grp1_dtp_mod, by=c("days_prior", "day_type", "prod_group_perc")) %>% 
  mutate(fcst_svv = OTB-OTB*predict_cxl_rate)
n_dp_grp1_dtp_fcs
```
```{r}
...
```{r}
# forecast otb to survive
n_dp_grp2_dtp_fcs <- left_join(h2_train_2, n_dp_grp2_dtp_mod, by=c("days_prior", "day_type", "prod_group_behav")) %>% 
  mutate(fcst_svv = OTB-OTB*predict_cxl_rate)
n_dp_grp2_dtp_fcs
```
