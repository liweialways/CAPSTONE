...
- For each days_prior, there are 580 observations. So for this step, we don't truncate data in terms of days_prior.
avg_bookings
```{r}
...
- day_of_week 1, 5, 6, 7: lower avg_bookings and lower avg_cancels. Does this make sense? Shouldn't Monday, Friday, Saturday and Sunday be the most popular days for hotel? Why the bookings are lower than other days? 
- group into 1567 and 234 two categories? 
...
h1_train %>%
  ...
  group_by(week) %>%
  summarise(avg_bookings = mean(daily_gross_bookings)) %>%
  ggplot(aes(x = reorder(week, -avg_bookings), y = avg_bookings)) +
           geom_bar(stat = "identity", position = "dodge") +
           theme_minimal() +
           theme(axis.title = element_blank(),
                axis.line.y = element_blank(),
