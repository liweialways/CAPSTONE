...
h1_train %>% 
  ...
  column_spec(1:2, color = "#000000")
```
Since other and opaque has such a small amount of observations, these should definitely be joined with other product types. The third grouping method is based on the cancelation rate curve above. If the cxl curves are similar, we group the types into one.

-group 1: government 199, other	22
-group 2: unfenced 741, membership marketing 260, business travel agencies 130
-group 3: corporate 1266, fenced 124, 
-group 4: opaque 78
-group 5: group 3524
...
mod1 <- lm(OTB_to_be_cxl~days_prior+day_of_week+month+prod_group_perc+cummulative_cxl_bookings, data=h1_train_1)
mod2 <- lm(OTB_to_be_cxl~days_prior+day_of_week+month+prod_group_perc+cummulative_gross_bookings, data=h1_train_1)
mod3 <- lm(OTB_to_be_cxl~days_prior+day_of_week+month+prod_group_perc+cummulative_gross_rev, data=h1_train_1)
mod4 <- lm(OTB_to_be_cxl~days_prior+day_of_week+month+prod_group_perc+OTB, data=h1_train_1)
...
```{r}
# Grouping Method 2:   
# h1_train_2: prod_group_behav, dow, days_prior, one of (cummulative_gross_bookings,cummulative_gross_rev,cummulative_cxl_bookings,cummulative_cxl_rev,OTB,OTB_rev)
h1_train_2$month <- month(h1_train_2$stay_dt)

mod1 <- lm(OTB_to_be_cxl~days_prior+day_of_week+month+prod_group_behav+cummulative_cxl_bookings, data=h1_train_2)
mod2 <- lm(OTB_to_be_cxl~days_prior+day_of_week+month+prod_group_behav+cummulative_gross_bookings, data=h1_train_2)
mod3 <- lm(OTB_to_be_cxl~days_prior+day_of_week+month+prod_group_behav+cummulative_gross_rev, data=h1_train_2)
mod4 <- lm(OTB_to_be_cxl~days_prior+day_of_week+month+prod_group_behav+OTB, data=h1_train_2)
...
```{r}
# Grouping Method 3:     
# h1_train_3: prod_group_cxl_rate, dow, days_prior, one of (cummulative_gross_bookings,cummulative_gross_rev,cummulative_cxl_bookings,cummulative_cxl_rev,OTB,OTB_rev)
...
```
-lm(formula = OTB_to_be_cxl ~ days_prior + day_of_week + month + 
    prod_group_cxl_rate + cummulative_cxl_bookings, data = h1_train_3) --- all the variables are statistically significant. 
...
If we solely rely on the cxl rate curve and regroup:
-group 1: government 124
-group 2: business travel agencies 836
-group 3: wholesale 695, corporate 3113, unfenced 3012, membership marketing 883
-group 4: group 2831, opaque 761, tactical marketing 100, fenced 170, other 5
...
All of the linear regression looks pretty good to predicting OTB_to_be_cxl. All statistically significant. 